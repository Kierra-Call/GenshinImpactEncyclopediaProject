...
async function getWeaponData(name) {
  ...
  weaponImageBackChange(weaponData.rarity);
}
...
function starChange2(rarity) {
  ...
  function weaponImageBackChange(rarity) {
    var myElement = document.getElementById("dynamicWeapon")
    console.log(myElement);
    myElement.classList.remove("star3-weapon","star1-weapon","star4-weapon","star5-weapon","star2-weapon");
    myElement.classList.add(`star${rarity}-weapon`);
  }
...
showSlides();
  ...
  async function getArtifactData(name) {
    var response = await fetch(`https://api.genshin.dev/artifacts/${name}`);
    var artifactData = await response.json();
  
    var artifactName = document.querySelector("#artifactName");
    artifactName.innerText = artifactData.name;
    var maxRarity = document.querySelector("#maxRarity");
    maxRarity.innerText = "Max Rarity: " + artifactData.max_rarity;
    var pieceArt2 = document.querySelector("#pieceArt2");
    pieceArt2.innerText = artifactData['2-piece_bonus'];
    var artifactName = document.querySelector("#pieceArt4");
    artifactName.innerText = artifactData['4-piece_bonus'];
imageChangeArtifact(name)
  }

  function imageChangeArtifact(name) {
    var photo = document.querySelector("#artifactImg");
    photo.src = `./assets/artifacts/${name}.png`;
    photo.alt = name;
  }

  function removeDisclaimerBar() {
    let element = document.querySelector("#disclaimerBox");
    element.remove();
  }